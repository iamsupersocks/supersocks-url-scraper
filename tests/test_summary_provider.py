...
def test_local_provider_skips_external_http(monkeypatch: pytest.MonkeyPatch) -> None:
    called = {"urlopen": False}

    def fake_urlopen(*args: object, **kwargs: object):
        called["urlopen"] = True
        raise AssertionError("local provider must not call external summary endpoints")
...
def test_unsupported_summary_provider_raises() -> None:
    with pytest.raises(Exception, match="unsupported summary provider"):
