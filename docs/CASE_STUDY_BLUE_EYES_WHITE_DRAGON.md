...
Public, bounded price case study for **Blue-Eyes White Dragon** /
...
Future hero visual (path reserved; binary not shipped in this change):
[`docs/assets/blue-eyes-supersocks-case-study.png`](assets/blue-eyes-supersocks-case-study.png).
...
**Read this case study edition-first.** Blue-Eyes / Dragon Blanc is not one SKU.
...
Show how to use the existing `supersocks-url-scraper` HTTP → SEO → browser fetch
primitives to collect a public, anonymized price snapshot for Blue-Eyes White
Dragon / Dragon Blanc aux Yeux Bleus, then read that snapshot honestly:
...
This section is the only place in this case study that names the marketplace
...
[Provider-string audit](#provider-string-audit).
...
11. **Publish** — JSON on stdout; optional sanitized CSV of floors; this public
    case study and SVGs keep **aggregates / public paths only**; any local
...
- Plain HTTP often returns 403; browser fallback may still render usable HTML
  while reporting a Cloudflare 403/429 status — the case study must keep both facts.
...
1. Open the Versions page (source URL 1 above) or jump via a known
...
Fixtures are synthetic HTML only. Passing pytest does not mean a live URL
fetch succeeded.
...
| --- | --- | --- |
| Live snapshot in this case study | Dated aggregates from the 2026-08-04 CEST run | Tomorrow’s market or HTTP friendliness |
...
| --- | --- | --- |
| Bounded provenance prose | This file § Source et provenance | Name the real provider once, honestly, without branding the whole dossier |
| Official / public URLs | Provenance tables; reproduction commands; CSV `public_product_path` cells; coverage/official manifests | Reproducibility requires the literal public endpoints |
