#!/usr/bin/env python3
"""Blue-Eyes White Dragon / Dragon Blanc market example on supersocks-url-scraper.

Parses publicly rendered marketplace HTML for Blue-Eyes White Dragon / Dragon
...
def is_access_challenge(markup: str) -> bool:
    ...
    page still exposes marketplace offer/version markup.
    """
    text = markup or ""
...
def collect_blue_eyes_white_dragon(
    urls: list[str],
    *,
    delay_seconds: float = 2.5,
    browser_fallback: bool = True,
...
def normalize_public_path(url_or_path: str) -> str:
    """Normalize a marketplace URL or path to a stable public path."""
...
def build_coverage_manifest(
    *,
    ...
    return {
        "title": "Blue-Eyes White Dragon public coverage manifest",
...
def render_coverage_manifest_markdown(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        "# Blue-Eyes White Dragon public coverage manifest",
        "",
...
def build_deep_enrichment_manifest(
    ...
    *,
    ...
    return {
        "title": "Blue-Eyes White Dragon public deep-enrichment manifest",
        "generated_at": utc_now_iso(),
        "timezone": ledger.get("timezone") or "UTC",
        "started_at": ledger.get("started_at"),
        "finished_at": ledger.get("finished_at") or utc_now_iso(),
...
def render_deep_enrichment_manifest_markdown(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        "# Blue-Eyes White Dragon public deep-enrichment manifest",
        "",
        f"- Generated: `{manifest.get('generated_at')}` ({manifest.get('timezone')})",
        f"- Stop reason: **{manifest.get('stop_reason')}**",
...
def validate_official_catalog_url(url: str, *, expected_url: str) -> str:
    """Refuse any URL that is not the exact canonical HTTPS official catalog S3 download."""
    if url != expected_url:
        raise ValueError("refusing non-canonical official catalog URL")
    parsed = urlparse(url)
...
def build_official_join_manifest(
    *,
    ...
    return {
        "title": "Blue-Eyes White Dragon official catalog join manifest",
        "generated_at": utc_now_iso(),
        "timezone": "UTC",
        "source_date": source_date,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Blue-Eyes White Dragon / Dragon Blanc public market extraction example (no seller identities, no HTML dumps)"
    )
    parser.add_argument(
        "--url",
        action="append",
        default=[],
        help="Public product URL to collect (repeatable). Prefer Versions + product pages.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Join official products_singles_3 + price_guide_3 catalogs for exact "
    ...
    if args.official_catalog_join:
        ...
        html_csv = args.html_coverage_csv or (
            repo_root / "docs" / "data" / "blue-eyes-white-dragon-coverage-2026-08-04.csv"
        )
    ...
    if args.deep_enrichment:
        ...
        checkpoint = args.deep_checkpoint or (
            repo_root / "runs" / "blue-eyes-white-dragon-deep" / "ledger.json"
        )
        resume = None
        ...
        payload = collect_blue_eyes_white_dragon(
            urls,
        ...
        if isinstance(payload["warnings"], list):
            payload["warnings"] = sorted(
                ...
                | {
                    ...
                    "public_product_path values are public marketplace URLs/paths, not printed card codes",
                }
            )
    ...
    if args.export_coverage_csv is not None or args.export_coverage_manifest is not None or args.coverage_crawl:
        ...
        if prior_csv is None:
            default_prior = (
                ...
                / "docs"
                / "data"
                / "blue-eyes-white-dragon-version-floors-2026-08-04.csv"
            )
